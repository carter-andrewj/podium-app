...
// SEARCH
	...
	search(force=false) {
		...
		const target = this.getState("search")
		if (target.length > 0) {
			this.timer = setTimeout(
				() => this.updateState(
					...
					() =>  this.props.store.api
						.search(target, this.getState("searchFilter"))
						.then(result => this.updateState(state => state
							.update("loading", l => l - 1)
							.update("results", current => 
								(this.getState("search") === target) ?
									result : current
							)
						))
						.catch(console.error)
...
// RESULTS
	...
	empty() {
		...
		if (this.getState("focussed")) {

			// Return results component
			return <View style={styles.quickSearch.empty}>
				<Text style={styles.quickSearch.emptyText}>
					{this.getState("search").length === 0 ?
						""
					: (this.getState("loading") || this.getState("changed")) ?
						"searching"
					:
						"no results"
					}
		...
		return <View style={styles.quickSearch.container}>
					...
					<View style={styles.quickSearch.overlay}>
						...
						<View style={styles.quickSearch.icon}>
							<FontAwesomeIcon
								icon="times"
								size={settings.layout.quickSearchIcon}
								color={this.getState("search").length > 0 ?
									settings.colors.bad :
									"transparent"
								}
			...
			<View style={[
				...
				<FlatList
					...
					refreshing={this.getState("loading") > 0}
